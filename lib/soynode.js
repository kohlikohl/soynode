...
var options = {
    ...
    // To set the base namespace for templates e.g. 'views' for templates in the namespace views.pages
    , namespace: ''

    // The directory into which closure styled views should be saved to
    , closureOutputDir: ''
}
...
function compileTemplates(inputDir, callback) {
    findFiles(inputDir, 'soy', function (err, files) {
        ...
        if (options.useClosureStyle) {
            compileClosureViews(inputDir, files);
        }
...
function compileClosureViews(inputDir, files){
    var args = [
        '-jar', PATH_TO_SOY_JAR
        , '--codeStyle', 'concat'
        , '--shouldGenerateJsdoc'
        , '--outputPathFormat', options.closureOutputDir + '/{INPUT_DIRECTORY}{INPUT_FILE_NAME}.js'
        , '--shouldProvideRequireSoyNamespaces'
    ]

    args = args.concat(files)

    console.log(inputDir, options.closureOutputDir, args);
    console.log(files);
    // Execute the comamnd inside the input directory.
    var cp = spawn('java', args, {cwd: inputDir})
...
/**
 * Adds compatibility with the express.js framework
 * @param path
 * @param data
 * @param callback
 */
function express(path, data, callback) {
    try{
        ...
        var templateName = [ options.namespace, this.name.replace('/','.'), data.template ].join('.');
        var temp = render(templateName, data, data.settings);

        if(typeof data.settings.layout !== 'undefined' && data.settings.layout !== ''){
            var layoutTemplate = [ options.namespace, data.settings.layout.replace('/','.'), 'index' ].join('.');
            data.content = temp;
            temp = render(layoutTemplate, data, data.settings);
        }

        callback(null, temp);
    } catch (e) {
        console.warn('soynode: Error rendering template ' + path, e)
        callback('soynode: Error rendering template ' + path);
    }
}
